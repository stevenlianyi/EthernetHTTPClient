...
public:
    ...
    {
        return std::unique_ptr<EthernetClient>(new EthernetClientSecure());
    }

    bool verify(EthernetClient& client, const char* host) override
    {
         EthernetClientSecure& wcs = static_cast<EthernetClientSecure&>(client);
...
bool EthernetHTTPClient::connect(void)
{
    ...
    // set Timeout for EthernetClient and for Stream::readBytesUntil() and Stream::readStringUntil()
